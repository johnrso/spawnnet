...
def make_grid(array, ncol=5, padding=0, pad_value=120):
    ...
    for i in range(nrow):
        row = np.pad(
            array[idx], [[padding if i == 0 else 0, padding], [padding, padding], [0, 0]],
            constant_values=pad_value, mode='constant')
        for j in range(1, ncol):
            idx += 1
            cur_img = np.pad(
                array[idx], [[padding if i == 0 else 0, padding], [0, padding], [0, 0]],
                constant_values=pad_value, mode='constant')
...
def make_grid_video_from_numpy(video_array, ncol, output_name='./output.mp4', speedup=1, fps=24):
    videos = []
